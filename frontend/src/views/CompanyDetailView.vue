...
async function saveEditNote(callId: string): Promise<void> {
  try {
    await callsStore.updateCallNote(callId, editingNotes.value);
    editingCallId.value = null;
    toast.success('Poznámka aktualizovaná');
    // Refresh history to show audit entry
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div v-if="editingCallId === log.id" class="mt-2">
                    ...
                    <div class="flex gap-2 mt-1">
                      <button class="text-xs text-primary-600 hover:text-primary-800 font-medium" @click="saveEditNote(log.id)">Uložiť</button>
                      <button class="text-xs text-gray-500 hover:text-gray-700" @click="cancelEditNote">Zrušiť</button>
                    </div>
                  </div>
                  <div v-else class="flex items-start gap-1 mt-1">
                    <p v-if="log.notes" class="text-sm text-gray-500 italic">{{ log.notes }}</p>
                    <p v-else class="text-xs text-gray-300 italic">Bez poznámky</p>
                    <button
                      class="text-gray-300 hover:text-primary-600 flex-shrink-0 ml-1 mt-0.5"
