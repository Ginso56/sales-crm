...
<template>
  <DashboardShell>
    <div class="mb-6">
      <h1 class="text-2xl font-bold text-gray-900">Prehľad</h1>
      <p class="text-gray-500 mt-1">Vitajte späť, {{ authStore.userName }}</p>
    </div>
    ...
    <template v-else-if="stats">
      ...
      <div v-if="goalBehind" class="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg flex items-center gap-3">
        ...
        <p class="text-sm text-red-800 font-medium">Zaostávate za cieľmi! Menej ako 50% splnených po polovici obdobia.</p>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        />
        <StatCard
          title="Hovory tento týždeň"
          :value="stats.callsWeek"
          icon="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"
          color="green"
        />
        <StatCard
          title="Hovory tento mesiac"
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div class="card">
          ...
          <p class="text-center text-sm text-gray-500 mt-3">
            {{ stats.answeredMonth + stats.noAnswerMonth > 0 ? Math.round(stats.answeredMonth / (stats.answeredMonth + stats.noAnswerMonth) * 100) : 0 }}% úspešnosť
          </p>
        </div>
